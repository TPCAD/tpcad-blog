...
选择排序的思想是遍历未排序元素，选择其中最小的元素，将其移动到已排序元素的末尾。
...
在未排序元素中依次比较相邻两个元素，若乱序则交换两个元素，这样一来较大的元素就会被慢慢移动到末尾有序排列。通常在冒泡排序中设置一个 `flag`，当一轮循环中没有发生交换操作时，说明数组已经有序，停止遍历。
...
```c
// 原数组
arr = [3, 4, 1, 9, 5, 8, 0] // 第一轮，arr[0] = 3 < arr[1] = 4，不交换

arr = [3, 4, 1, 9, 5, 8, 0] // 第一轮，arr[1] = 4 > arr[2] = 1，交换

arr = [3, 1, 4, 9, 5, 8, 0] // 第一轮，arr[2] = 4 < arr[3] = 9，不交换

arr = [3, 1, 4, 9, 5, 8, 0] // 第一轮，arr[3] = 9 > arr[4] = 5，交换

arr = [3, 1, 4, 5, 9, 8, 0] // 第一轮，arr[4] = 9 > arr[5] = 8，交换

arr = [3, 1, 4, 5, 8, 9, 0] // 第一轮，arr[5] = 9 > arr[6] = 0，交换
// 第一轮结束，最大值 9 被移动到数组尾部
arr = [3, 1, 4, 5, 8, 0, 9]
...
上面的实现每次都要比较到数组的最后，而每次遍历之后尾部的元素都是有序的，并不需要再进行比较，因此可以记录上一轮循环最后一次交换操作的位置，下一轮循环就可以在该位置结束，因为后面的元素已经是有序的了。
